...
from compare_flann import image_distance as image_distance2
...
def get_flann_dists(handle, dist):
	for new_line in handle:
		img1, img2 = new_line.strip().split()
		...
		yield image_distance2(img1_file, img2_file)
...
if __name__ == '__main__':
	...
	with open(same_tests, 'r') as same_handle, open(diff_tests, 'r') as diff_handle:
		same_dists_euclidean = np.array(list(get_flann_dists(same_handle, euclidean)))
		diff_dists_euclidean = np.array(list(get_flann_dists(diff_handle, euclidean)))
		
		plt.hist(same_dists_euclidean, color='green', range = [0, 25000])
		plt.hist(diff_dists_euclidean, color='red', range = [0, 25000])
